...
<div style="display:flex;gap:0.5rem;margin-bottom:1.25rem;border-bottom:1px solid var(--clr-border);padding-bottom:0.75rem;">
  <a href="transport.php?tab=commercial" class="btn btn-sm <?= $tab === 'commercial' ? 'btn-primary' : 'btn-ghost' ?>">🚌 Commercial Transport</a>
  <a href="transport.php?tab=mbanda" class="btn btn-sm <?= $tab === 'mbanda' ? 'btn-primary' : 'btn-ghost' ?>">🚗 Mbanda Ride Sharing</a>
</div>
      ...
      <tbody>
        ...
        <?php else: ?>
          ...
          <tr>
            ...
            <td>
              <div style="font-weight:600;"><?= e($trip['pickup_location']) ?> → <?= e($trip['destination']) ?></div>
        ...
        <?php else: ?>
          ...
          <tr id="t-row-<?= e($l['id']) ?>">
            ...
            <td>
              ...
              <div class="text-xs text-muted">📍 <?= e($l['location']) ?></div>
            </td>
            <td class="text-xs"><?= e($l['vendor_name']) ?></td>
            <td class="text-xs">
              <?= e($meta['origin'] ?? $meta['departure_city'] ?? '—') ?>
              <?php if (!empty($meta['destination'])): ?> → <?= e($meta['destination']) ?><?php endif; ?>
            </td>
            <td>
          ...
          <tr id="seat-manage-<?= e($l['id']) ?>" style="display:none;">
            <td colspan="8" style="background:var(--clr-surface2);padding:1.25rem;">
              <h4 style="margin-bottom:0.75rem;color:var(--clr-cyan);">💺 Seat Classes — <?= e($l['title']) ?></h4>
              <?php if (count($seats) > 0): ?>
