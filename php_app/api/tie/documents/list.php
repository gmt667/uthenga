...
try {
    ...
    $filters = ['category' => (string) ($query['category'] ?? ''), 'trip_id' => (string) ($query['trip_id'] ?? '')];
    UthengaTieApi::respond(['success' => true, 'request_id' => $requestId, 'result' => (new UthengaTieKernel())->customerDocuments->list($user['id'], $filters)]);
