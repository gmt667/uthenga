...
try {
    ...
    $status = strtolower(trim((string) (UthengaTieApi::query()['status'] ?? '')));
    UthengaTieApi::respond(['success' => true, 'request_id' => $requestId, 'result' => (new UthengaTieKernel())->customerBookings->list($user['id'], $status)]);
